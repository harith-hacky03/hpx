...
int main()
{
    ...
    {
        std::uint32_t n = 0;
        run_on_all<std::uint32_t>(
            reduction_plus(n), [](std::uint32_t& local_n) { ++local_n; });
        HPX_TEST_EQ(n, static_cast<std::uint32_t>(hpx::get_num_worker_threads()));
    }

    // Test with specific number of tasks
    {
        std::uint32_t n = 0;
        run_on_all<std::uint32_t>(hpx::execution::par, 2, reduction_plus(n),
    ...
    {
        std::uint32_t n = 0;
        run_on_all<std::uint32_t>(hpx::execution::seq, reduction_plus(n),
            [](std::uint32_t& local_n) { ++local_n; });
        HPX_TEST_EQ(n, static_cast<std::uint32_t>(hpx::get_num_worker_threads()));
    }
    ...
    {
        std::uint32_t n = 0;
        run_on_all<std::uint32_t>(hpx::execution::par, reduction_plus(n),
            [](std::uint32_t& local_n) { ++local_n; });
        HPX_TEST_EQ(n, static_cast<std::uint32_t>(hpx::get_num_worker_threads()));
    }
    ...
    {
        std::uint32_t n = 0;
        run_on_all<std::uint32_t>(hpx::execution::par_unseq, reduction_plus(n),
            [](std::uint32_t& local_n) { ++local_n; });
        HPX_TEST_EQ(n, static_cast<std::uint32_t>(hpx::get_num_worker_threads()));
    }
    ...
    // Test with multiple arguments
    {
        std::uint32_t n = 0;
        std::uint32_t m = 0;
        run_on_all<std::uint32_t, std::uint32_t>(reduction_plus(n), reduction_plus(m),
            [](std::uint32_t& local_n, std::uint32_t& local_m) {
                ++local_n;
                local_m += 2;
            });
        HPX_TEST_EQ(n, static_cast<std::uint32_t>(hpx::get_num_worker_threads()));
        HPX_TEST_EQ(m, static_cast<std::uint32_t>(2 * hpx::get_num_worker_threads()));
    }

    // Test with vector reduction
    {
        std::vector<std::uint32_t> v(hpx::get_num_worker_threads(), 0);
        run_on_all<std::vector<std::uint32_t>>(reduction_plus(v), 
            [](std::vector<std::uint32_t>& local_v) {
    ...
    // Test with atomic operations
    {
        ...
        run_on_all([&]() { ++n; });
        HPX_TEST_EQ(n.load(), static_cast<std::uint32_t>(hpx::get_num_worker_threads()));
    }

    // Test with different number of tasks
    {
        std::uint32_t n = 0;
        run_on_all<std::uint32_t>(hpx::execution::par, 1, reduction_plus(n),
            [](std::uint32_t& local_n) { ++local_n; });
        HPX_TEST_EQ(n, static_cast<std::uint32_t>(1));

        n = 0;
        run_on_all<std::uint32_t>(hpx::execution::par, 4, reduction_plus(n),
            [](std::uint32_t& local_n) { ++local_n; });
